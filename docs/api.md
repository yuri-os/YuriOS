...
| `GET /api/history?limit=&before=` | `{messages, has_more}`, oldest first. No arguments is the end of the conversation (100 entries) — what a fresh page backfills, and it survives a restart: the ring is seeded from `<vault>/state/transcript.jsonl`. `before=<message id>` is the `limit` entries just *older* than that one — the walk back at the top of the column, six a press. `limit` is capped at 200 |
