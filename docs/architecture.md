...
| **What a tick can do** | `mind/acts.py` · `mind/goalwork.py` · `mind/prompts.py` · `mind/housekeeping.py` |
...
| **The mind debug page** | `world/debug.py` (one log at a time) + `world/debug_graph.py` (the joined graph) + `web/mind/` (host routes under `…/debug/*`) |
...
./scripts/check.sh                 # lint, typecheck, both suites (~60s across 8 workers)
./scripts/check.sh --fast          # …without pytest
